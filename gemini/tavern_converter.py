...
将 SillyTavern JSON 格式的预设文件转换为 gemini 模块的 YAML preset 格式。
"""
...
DEFAULT_JSON_DIR = PRESET_MODULE_ROOT / "json"
DEFAULT_PRESET_DIR = PRESET_MODULE_ROOT / "preset"
...
def convert_tavern_to_preset(json_path: str | Path, output_path: str | Path) -> bool:
    ...
    logger.info("转换: %s -> %s", json_path.name, output_path.name)
    ...
    try:
        ...
        logger.error("JSON 解析失败: %s - %s", json_path, exc)
        raise

    # 提取 prompts
    prompts = data.get("prompts", [])
    if not prompts:
        logger.warning("未找到 prompts: %s", json_path.name)
        return False
    ...
    try:
        with output_path.open("w", encoding="utf-8") as f:
            ...
            if generation_params:
                logger.debug("跳过 generation 参数: %s", generation_params)

        logger.info("✓ 转换成功: %s", output_path.name)
        return True

    except Exception as exc:  # noqa: BLE001
        logger.error("写入 YAML 失败: %s - %s", output_path, exc)
        return False
...
def batch_convert(
    ...
    if not json_files:
        logger.warning("未找到 JSON 文件: %s", json_dir)
        return {
            "total": 0,
            "success": 0,
            "failed": 0,
    ...
    for json_file in json_files:
        ...
        if output_file.exists() and not overwrite:
            logger.info("⊙ 跳过已存在: %s", output_file.name)
            stats["skipped"] += 1
        ...
        try:
            ...
            logger.error("✗ 转换失败: %s - %s", json_file.name, exc)
            stats["failed"] += 1
            stats["files"]["failed"].append(json_file.name)

    # 输出统计
    logger.info("=" * 60)
    logger.info("转换完成: 成功 %d, 失败 %d, 跳过 %d", stats["success"], stats["failed"], stats["skipped"])
    logger.info("=" * 60)
