...
class GeminiFormatHandler:
    ...
    支持两种格式约束模式：
    1. 平台原生支持（json/json_schema）：使用 generation_config 的 response_mime_type 和 response_schema
    2. 提示词工程（markdown/text）：将格式要求合并到用户消息中
    """

    @staticmethod
    def get_generation_config(cfg: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """
        根据 format 配置生成 Gemini 的 generation_config 参数。

        仅用于平台原生支持的格式（json/json_schema）。
        """
        if not cfg:
            return None
        ...
        # markdown/text 等其他格式不需要 generation_config
        return None

    @staticmethod
    def get_prompt_suffix(cfg: Optional[Dict[str, Any]]) -> Optional[str]:
        """
        根据 format 配置生成需要附加到用户消息末尾的提示词。
        """
        if not cfg:
            return None
        ...
        # json/json_schema 使用平台原生支持，不需要提示词
        return None
